...
public:
  p_fun(mpi_tree *model, int last_branch_in, communicator world_in,
        double delta_start = 0.01, double tau_start = 0.01,
        double lambda_start = 0.01)
    ...
    IntervalConstraint *constraint =
        new IntervalConstraint(1e-10, 1e3, true, true);
...
int main(int argc, char **argv) {
  ...
  if (atoi(argv[3]) == 1)
    infer_tree->model->set_model_parameter("undatedBL", true);
  else
    infer_tree->model->set_model_parameter("undatedBL", false);
  ...
  if (argc < 7) {
    ...
    if (world.rank() == 0)
      cout << "#ML rate optimization.." << endl;
    ...
    if (world.rank() == 0)
      cout << "#skipping with: delta=" << delta << " lambda=" << lambda
           << " tau=" << tau << endl;
  }
  // optimizer->getParameters().printParameters(cout);
  if (argc > 7)
  ...
  samples = atoi(argv[8]);
  if (world.rank() == 0)
    cout << "#rates : delta=" << delta << " lambda=" << lambda << " tau=" << tau
         << endl;
  ...
  if (world.rank() == 0)
    cout << "#gather T_from done." << endl;

  if (world.rank() == 0)
    cout << ">tree:\t" << infer_tree->model->string_parameter["S_with_ranks"]
         << endl;
  if (world.rank() == 0)
    cout << ">logl:\t" << ll_final << endl;
  if (world.rank() == 0)
    cout << ">Ts:\tfrom\tto" << endl;
  if (world.rank() == 0)
    infer_tree->print_branch_counts(samples);
  return 0;
  if (world.rank() == 0)
      ...
      if (Ts > 0)
        for (int i = 0; i < (*it).second.size(); i++) {
          ...
          if (e < infer_tree->model->last_leaf)
            cout
                << "\t"
                << infer_tree->model->node_name[infer_tree->model->id_nodes[e]];
          else
            cout << "\t" << e;
          if (f < infer_tree->model->last_leaf)
            cout
                << "\t"
                << infer_tree->model->node_name[infer_tree->model->id_nodes[f]];
          else
            cout << "\t" << f;
